...
const releaseNotes = read("docs/release-notes/SwarmSim-Strategy-Autobuyer-5.0.0-release-notes.md");
...
const checks = [
  ...
  [status.includes("Milestone 5") && status.includes("5.0.0"), "status board must mention M5 and 5.0.0"],
  [releaseNotes.includes(`# SwarmSim Strategy Autobuyer ${VERSION}`), "release-note title must match"],
  [releaseNotes.includes("Ascension and Mutagen advisor"), "M5 release-note capability must be documented"],
];

const failures = checks.filter(([ok]) => !ok).map(([, message]) => message);
if (failures.length) {
  console.error(`FAIL check-5.0.0-version-surfaces: ${failures.join("; ")}`);
  process.exit(1);
}

console.log("PASS check-5.0.0-version-surfaces");
