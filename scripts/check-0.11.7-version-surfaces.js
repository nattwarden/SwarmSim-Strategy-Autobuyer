...
const scenarioResultsTemplatePath = path.join(root, "docs", "test-data", "0.11.7-scenarios", "scenario-results.json");
...
if (pkgVersion !== "0.11.7") fail(`F7 failed: package version must be 0.11.7, got ${pkgVersion || "missing"}.`);

requirePattern(canonicalSource, /^\/\/\s*@version\s+0\.11\.7$/mu, "F7 failed: userscript metadata version must be 0.11.7.");
requirePattern(source, /const\s+AUTOBUYER_VERSION\s*=\s*"0\.11\.7"\s*;/, "F7 failed: canonical runtime version constant must be 0.11.7.");
...
if (fs.existsSync(scenarioResultsTemplatePath)) {
  ...
  if (!scriptVersion || scriptVersion !== "0.11.7") {
    fail(`F8 failed: scenario-results template scriptVersion must be 0.11.7, got ${scriptVersion || "missing"}.`);
  }
  if (!autobuyerVersion || autobuyerVersion !== "0.11.7") {
    fail(`F8 failed: scenario-results template autobuyerVersion must be 0.11.7, got ${autobuyerVersion || "missing"}.`);
  }
}

console.log("0.11.7 version surfaces checks passed (F7-F8).");
