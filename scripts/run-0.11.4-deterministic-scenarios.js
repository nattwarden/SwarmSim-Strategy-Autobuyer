...
const inputPath = path.join(root, "docs", "test-data", "0.11.4-scenarios", "scenario-definitions.json");
const outputPath = path.join(root, "docs", "test-data", "0.11.4-scenarios", "scenario-results.json");
const summaryPath = path.join(root, "docs", "test-data", "0.11.4-scenarios", "scenario-summary.md");
...
function buildSummaryMarkdown(report) {
  const lines = [];
  lines.push("# 0.11.4 Deterministic Scenario Summary");
  lines.push("");
  lines.push(`- Run at: ${report.runAt || "not-run"}`);
  lines.push(`- Source: ${report.source || "deterministic-scenario"}`);
  lines.push(`- Script version: ${report.scriptVersion || "0.11.4"}`);
  lines.push(`- Scenario report version: ${report.scenarioReportVersion || "0.11.4"}`);
  lines.push("- Invariants: not-run");
  lines.push("");
  lines.push("This is an empty template for the 0.11.4 browser run.");
...
function main() {
  ...
  const report = {
    ...
    scriptVersion: String(definitions.version || "0.11.4"),
    scenarioReportVersion: "0.11.4",
    runAt: null,
