...
const releaseNotes = read("docs/release-notes/SwarmSim-Strategy-Autobuyer-4.0.0-release-notes.md");
...
const checks = [
  ...
  [releaseNotes.includes("major version follows the active BOOK-00 milestone"), "milestone version policy must be documented"],
];

const failures = checks.filter(([ok]) => !ok).map(([, message]) => message);
if (failures.length) {
  console.error(`FAIL check-4.0.0-version-surfaces: ${failures.join("; ")}`);
  process.exit(1);
}

console.log("PASS check-4.0.0-version-surfaces");
