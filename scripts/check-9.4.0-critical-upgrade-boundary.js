...
function getBoundaryRow(cycle, label) {
  ...
  for (const otherRow of coverage.paths || []) {
    ...
    const expectedContract = DECLARED_BOUNDARY_CONTRACTS[otherRow.pathId] || null;
    assert(otherRow.boundaryContract === expectedContract, `${label}: ${otherRow.pathId} declares boundary contract ${otherRow.boundaryContract}, expected ${expectedContract}`);
    if (!expectedContract) {
